...
function draw() {
  ...
  for (let ballNumber = 0; ballNumber < count_of_balls; ballNumber++) {
    console.log("Intialized");
    ballFired[ballNumber].move();
    ballFired[ballNumber].display();
    ballFired[ballNumber].checkCollision();
  }
...
function keyPressed() {
  if (key === ' ') {
    bubbleFired = true;
    console.log(bubbleFired);

    ballFired.push(new FiringBall());
    count_of_balls += 1;
    ballFired[count_of_balls].initialize(fireCannon.x + cannonWidth/4, fireCannon.y, random(colours));
  }
}
...
class FiringBall extends Cannon {
  ...
  initialize(x, y, colour) {
    this.x = x;
    this.y = y;
    this.colour = colour;
    this.inertia = this.direction/5
    this.fired = true;
    this.kill = false;
  }
  ...
  checkCollision() {
    for (let row of grid) {
      for (let cell of row) {
        if (cell.active && dist(this.x, this.y, cell.xPosition, cell.yPosition) < GRID_SIZE) {
          this.fired = false;
          cell.active = false; 
          bubbleFired = false; 
          this.kill = true;
